...
//        countCitiesOfRegion(cities);
//        showCitiesOfTheRegion(cities);
        countCitiesOfRegions(cities);
    }
    ...
    public static void countCitiesOfRegion (List<City> cities) {
        int count = 0;
        String targetRegion = "Вологодская область";
        for (City city : cities) {
            if (city.getRegion().compareToIgnoreCase(targetRegion) == 0) {
                count ++;
            }
        }
        System.out.println(targetRegion + " - " + count);
    }

    public static void showCitiesOfTheRegion (List<City> cities) {
       String region = "Карелия";
       int count;
       cities.stream().filter(city -> (city.getRegion().compareToIgnoreCase(region) == 0)).forEach(System.out::println);
    }
}
